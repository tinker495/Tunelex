"""Utility for plotting KMNIST experiment metrics stored under logs/."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot KMNIST experiment metrics from CSV logs")
    parser.add_argument(
        "--logs-dir",
        type=Path,
        default=Path("logs/kmnist"),
...
def _collect_csv_files(logs_dir: Path) -> list[Path]:
    ...
    csv_files = sorted(p for p in logs_dir.glob("kmnist_eval_metrics_*.csv") if p.is_file())
    if not csv_files:
        raise FileNotFoundError(f"No KMNIST metrics CSV files found in '{logs_dir}'")
    return csv_files


RUN_PATTERN = re.compile(
    r"^kmnist_eval_metrics_"
